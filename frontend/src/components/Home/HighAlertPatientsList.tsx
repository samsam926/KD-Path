...
import { AlertTriangle, Activity, Heart, Droplets, ChevronDown, ChevronUp, Eye } from 'lucide-react';
...
export function HighAlertPatientsList({ patients, expandedPatients, toggleExpand, getAlertLevel, getRiskLevel, onSelectPatient }: HighAlertPatientsListProps) {
    ...
    return (
      <Card className="p-12 text-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <div>
            <h3 className="font-semibold text-healthcare-primary mb-1">No High Alert Patients</h3>
                ...
                <div className="flex items-center gap-4 flex-1">
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      <AlertTriangle className={cn("h-5 w-5", alert.textColor)} />
                    </div>
                    <div>
            ...
            {isExpanded && (
              <div className="px-4 pb-4 pt-0 border-t border-gray-200">
                <div className="pt-4 space-y-4">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-healthcare-primary mb-3">Current Vital Signs</h4>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                          <Droplets className="h-3 w-3 text-healthcare-secondary" />
                          <span className="text-xs text-muted-foreground">eGFR</span>
                        </div>
                        <p className={cn(
                          "font-bold",
                          (patient.vitals?.eGFR || 0) < 30 ? 'text-alert-high' :
                          ...
                          {patient.vitals?.eGFR} <span className="text-xs font-normal">mL/min</span>
                        </p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                      ...
                      <div className="p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="font-bold text-healthcare-primary">
                          {patient.vitals?.oxygenSat} <span className="text-xs font-normal">%</span>
                        </p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="font-bold text-healthcare-primary">
                          {patient.vitals?.temperature} <span className="text-xs font-normal">°F</span>
                        </p>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-2 mb-1">
                          <Droplets className="h-3 w-3 text-healthcare-secondary" />
                          <span className="text-xs text-muted-foreground">ACR</span>
                        </div>
                        <p className="font-bold text-healthcare-primary">
                          {patient.vitals?.acr} <span className="text-xs font-normal">mg/g</span>
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Kidney Failure Risk */}
                  <div>
                    <h4 className="text-sm font-semibold text-healthcare-primary mb-3">Kidney Failure Risk</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="p-3 bg-alert-high/5 rounded-lg border border-alert-high/20">
                        <p className="text-xs text-muted-foreground mb-1">2-Year Risk</p>
                        <div className="flex items-center justify-between">
                          <p className="text-2xl font-bold text-alert-high">
                            {patient.vitals?.twoYearRisk?.toFixed(1)}%
                          </p>
                          <Badge className={`${twoYearRisk.color} text-white text-xs`}>
                            {twoYearRisk.level}
                          </Badge>
                        </div>
                      </div>
                      <div className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                        <p className="text-xs text-muted-foreground mb-1">5-Year Risk</p>
                        <div className="flex items-center justify-between">
                          <p className="text-2xl font-bold text-orange-600">
                            {patient.vitals?.fiveYearRisk?.toFixed(1)}%
                          </p>
                          <Badge className={`${getRiskLevel(patient.vitals?.fiveYearRisk).color} text-white text-xs`}>
                  ...
                  {patient.visits && patient.visits.length > 0 && (
                    <div>
                      <h4 className="text-sm font-semibold text-healthcare-primary mb-3">Recent Visits</h4>
                      <div className="space-y-2">
                        {patient.visits.slice(-2).reverse().map((visit) => (
                          <div key={visit.visitNumber} className="p-3 bg-healthcare-light/10 rounded-lg">
