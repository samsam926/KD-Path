...
export function QuickMetrics() {
  ...
  const raceDistribution = patients.reduce(
    (acc, patient) => {
  ...
  const fastCkdCount = patients.filter(
    (p) => p.status === "Fast Progression CKD",
  ).length
  const _uncertainCount = patients.filter(
    (p) => p.status === "FastCKD uncertainty",
  ).length

  const healthyPercent =
    totalPatients > 0 ? Math.round((healthyCount / totalPatients) * 100) : 0
  const ckdPercent =
    totalPatients > 0 ? Math.round((ckdCount / totalPatients) * 100) : 0
  const fastCkdPercent =
    totalPatients > 0 ? Math.round((fastCkdCount / totalPatients) * 100) : 0
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className={cn("p-3 rounded-lg", `bg-${metric.color}/10`)}>
