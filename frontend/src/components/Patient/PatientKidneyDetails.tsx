...
export function PatientKidneyDetails({ patient }: PatientKidneyDetailsProps) {
  const getRiskLevel = (twoYearRisk?: number): { level: string; color: string; bgColor: string } => {
    if (!twoYearRisk) return { level: 'Unknown', color: 'text-muted-foreground', bgColor: 'bg-muted' };
    if (twoYearRisk >= 20) return { level: 'Very High', color: 'text-alert-high', bgColor: 'bg-alert-high' };
    if (twoYearRisk >= 10) return { level: 'High', color: 'text-orange-600', bgColor: 'bg-orange-600' };
    if (twoYearRisk >= 5) return { level: 'Moderate', color: 'text-amber-600', bgColor: 'bg-amber-600' };
    if (twoYearRisk >= 2) return { level: 'Low-Moderate', color: 'text-yellow-600', bgColor: 'bg-yellow-600' };
  ...
  const getGFRStage = (gfr?: number): { stage: string; description: string } => {
    ...
    if (gfr >= 45) return { stage: 'G3a', description: 'Mild to moderate decrease' };
    if (gfr >= 30) return { stage: 'G3b', description: 'Moderate to severe decrease' };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 bg-gradient-to-br from-healthcare-primary/5 to-healthcare-primary/10 border-healthcare-primary/20">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="text-3xl font-bold text-healthcare-primary">{patient.age}</p>
        ...
        <Card className={`p-4 border-2 ${
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-1">mL/min/1.73m²</p>
        ...
        <Card className={`p-4 border-2 ${
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className={`text-3xl font-bold ${getACRColor(patient.vitals?.acr)}`}>
        ...
        <Card className={`p-4 border-2 ${
          ...
          <div className="flex items-start justify-between">
            ...
            <AlertTriangle className={`h-8 w-8 ${riskInfo.color.replace('text-', 'text-').concat('/40')}`} />
          </div>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-healthcare-primary">2-Year Risk</span>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-healthcare-primary">5-Year Risk</span>
              <span className={`text-2xl font-bold ${riskInfo.color}`}>
      ...
      <Card className="p-6">
        <h3 className="font-semibold text-healthcare-primary mb-4">Clinical Information</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* GFR Stages */}
          <div>
            <h4 className="text-sm font-semibold text-healthcare-primary mb-3">eGFR Stage Classification</h4>
            <div className="space-y-2">
              <div className={`p-3 rounded-lg border-l-4 ${
                gfrStage.stage === 'G1' ? 'bg-green-50 border-green-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">G1: ≥90</span>
                  <span className="text-xs text-muted-foreground">Normal or high</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                gfrStage.stage === 'G2' ? 'bg-green-50 border-green-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">G2: 60-89</span>
                  <span className="text-xs text-muted-foreground">Mildly decreased</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                gfrStage.stage === 'G3a' ? 'bg-amber-50 border-amber-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">G3a: 45-59</span>
                  <span className="text-xs text-muted-foreground">Mild-moderate decrease</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                gfrStage.stage === 'G3b' ? 'bg-amber-50 border-amber-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">G3b: 30-44</span>
                  <span className="text-xs text-muted-foreground">Moderate-severe decrease</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                gfrStage.stage === 'G4' ? 'bg-red-50 border-red-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">G4: 15-29</span>
                  <span className="text-xs text-muted-foreground">Severely decreased</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                gfrStage.stage === 'G5' ? 'bg-red-50 border-red-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">G5: {'<'}15</span>
                  <span className="text-xs text-muted-foreground">Kidney failure</span>
                </div>
              </div>
            </div>
            <div className="mt-3 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm">
                <span className="font-semibold text-healthcare-primary">Current Stage: </span>
                <span className="text-healthcare-secondary">{gfrStage.stage} - {gfrStage.description}</span>
              </p>
            </div>
          </div>
          ...
          {/* ACR Categories */}
          <div>
            <h4 className="text-sm font-semibold text-healthcare-primary mb-3">ACR Category Classification</h4>
            <div className="space-y-2">
              <div className={`p-3 rounded-lg border-l-4 ${
                (patient.vitals?.acr || 0) < 30 ? 'bg-green-50 border-green-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">A1: {'<'}30</span>
                  <span className="text-xs text-muted-foreground">Normal to mildly increased</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                (patient.vitals?.acr || 0) >= 30 && (patient.vitals?.acr || 0) < 300 
                  ? 'bg-amber-50 border-amber-500' 
                  : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">A2: 30-299</span>
                  <span className="text-xs text-muted-foreground">Moderately increased</span>
                </div>
              </div>
              <div className={`p-3 rounded-lg border-l-4 ${
                (patient.vitals?.acr || 0) >= 300 ? 'bg-red-50 border-red-500' : 'bg-gray-50 border-gray-300'
              }`}>
                <div className="flex justify-between items-center">
                  <span className="text-sm font-semibold">A3: ≥300</span>
                  <span className="text-xs text-muted-foreground">Severely increased</span>
                </div>
              </div>
            </div>
            <div className="mt-3 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm">
                <span className="font-semibold text-healthcare-primary">Current Category: </span>
                <span className="text-healthcare-secondary">{getACRCategory(patient.vitals?.acr)}</span>
              </p>
            </div>

            <div className="mt-6">
              <h4 className="text-sm font-semibold text-healthcare-primary mb-3">Risk Level Guide</h4>
              <div className="space-y-2">
